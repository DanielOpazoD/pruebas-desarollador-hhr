...
export const exportDataCSV = (record: DailyRecord | null) => {
    ...
    if (record.transfers && record.transfers.length > 0) {
        ...
        record.transfers.forEach(t => {
            ...
            const escort = t.evacuationMethod === 'Aerocardal' ? '' : (t.transferEscort || '');
            rows.push(`${t.bedName},${t.bedType},${escape(t.patientName)},${t.rut},${escape(t.diagnosis)},${t.evacuationMethod},${escape(center)},${escape(escort)},${t.age || ''},${t.insurance || ''}`);
        });
    }
