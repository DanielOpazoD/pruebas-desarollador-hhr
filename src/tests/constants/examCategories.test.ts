...
describe('EXAM_CATEGORIES', () => {
  ...
  describe('Bioquimica Category', () => {
    ...
    it('should not duplicate green-tube-only exams', () => {
      expect(bioquimica?.exams).not.toContain('ELECTROLITOS PLASMATICOS');
      expect(bioquimica?.exams).not.toContain('LACTATO');
    });

    it('should not include ferritina in bioquimica', () => {
      expect(bioquimica?.exams).not.toContain('FERRITINA');
    });
  });
