...
describe('patientRowOrbitalQuickActionsController', () => {
  it('builds visible orbital items with trigonometrically derived coordinates', () => {
    ...
    expect(items[0]).toMatchObject({ angleDeg: -60, x: 25, y: -43.3 });
    expect(items[1]).toMatchObject({ angleDeg: 0, x: 50, y: 0 });
    expect(items[2]).toMatchObject({ angleDeg: 60, x: 25, y: 43.3 });
  });
