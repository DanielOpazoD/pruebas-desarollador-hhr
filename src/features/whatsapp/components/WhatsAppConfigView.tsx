...
const WhatsAppConfigForm: React.FC<WhatsAppConfigFormProps> = ({
  ...
  const { data: groups = [], isLoading: loadingGroups } = useWhatsAppGroupsQuery(
    botStatus === 'connected'
  );
  ...
  const [autoSendTime, setAutoSendTime] = useState(
    initialConfig.handoffNotifications?.autoSendTime || '17:00'
  );
    ...
    <div className="space-y-6">
      ...
      <div
        className={`p-4 rounded-lg border-2 ${
          botStatus === 'connected' ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'
        }`}
      >
        <div className="flex items-center gap-3">
          {botStatus === 'connected' ? (
              ...
              <button
                onClick={() =>
                  setLocalConfig({
                    ...localConfig,
                    shiftParser: {
                      ...localConfig.shiftParser,
                      enabled: !localConfig.shiftParser.enabled,
                    },
                  })
                }
                className={`w-12 h-6 rounded-full transition-colors ${
                  localConfig.shiftParser.enabled ? 'bg-green-500' : 'bg-gray-300'
                }`}
              >
                <div
                  className={`w-5 h-5 rounded-full bg-white shadow transform transition-transform ${
                    localConfig.shiftParser.enabled ? 'translate-x-6' : 'translate-x-0.5'
                  }`}
                />
              </button>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full transition-colors ${
                  localConfig.handoffNotifications.enabled ? 'bg-green-500' : 'bg-gray-300'
                }`}
              >
                <div
                  className={`w-5 h-5 rounded-full bg-white shadow transform transition-transform ${
                    localConfig.handoffNotifications.enabled ? 'translate-x-6' : 'translate-x-0.5'
                  }`}
                />
              </button>
            </div>
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">Grupo para Enviar Entregas</label>
              <select
      ...
      <div className="bg-white border rounded-lg p-5 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          <div>
            <label className="block text-sm text-gray-600 mb-1">Hora de envío automático</label>
            <input
              type="time"
              value={autoSendTime}
