...
export const DataImportModal: React.FC<DataImportModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [status, setStatus] = useState<
    'IDLE' | 'READING' | 'VALIDATED' | 'IMPORTING' | 'SUCCESS' | 'ERROR'
  >('IDLE');
  ...
  const [results, setResults] = useState({
    ...
    failed: 0,
    repaired: 0,
    outcome: 'clean' as 'clean' | 'repaired' | 'partial' | 'blocked',
  });
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl p-10 hover:border-indigo-300 transition-colors bg-slate-50/30 group">
            ...
            <label htmlFor="backup-upload" className="cursor-pointer flex flex-col items-center">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Suelte el archivo aquí o haga clic para buscar
              </p>
            </label>
          </div>
        )}
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-4 flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-[11px] font-medium text-slate-500">
                  <span>
                    📂 Mes: {backupData.month}/{backupData.year}
                  </span>
            ...
            <div className="bg-amber-50 border border-amber-100 p-4 rounded-xl flex items-start gap-3">
              ...
              <p className="text-xs text-amber-800 leading-relaxed">
                <strong>Atención:</strong> La importación sobrescribirá los datos locales y remotos
                para los días incluidos en el archivo. Asegúrese de que este respaldo sea el
                correcto.
              </p>
            </div>
            ...
            {status === 'VALIDATED' && (
              <div className="flex gap-3 w-full pt-4 border-t border-slate-100">
                ...
                <button
                  ...
                  className="flex-[2] py-3 px-4 rounded-xl bg-indigo-600 text-sm font-bold text-white shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95"
                >
                  Iniciar Importación
                </button>
              </div>
        ...
        {status === 'IMPORTING' && (
          <div className="flex flex-col items-center py-6">
            ...
            <p className="text-sm text-slate-500 mt-1">
              Importando {progress.current} de {progress.total} días...
            </p>
        ...
        {status === 'SUCCESS' && (
          <div className="flex flex-col items-center py-6 text-center animate-scale-in">
            ...
            <p className="text-sm text-slate-500 mt-2">{summarizeBackupImportOutcome(results)}</p>
            <button
              onClick={onClose}
