...
import { AuditLogEntry, AuditDeviceChangesMap } from '@/types/audit';
                                        ...
                                        <div className="mt-3 bg-white/60 rounded-lg p-2.5 border border-slate-100/50 space-y-2">
                                            {Object.entries(log.details.changes).map(([field, delta]) => {
                                                if (field === 'deviceDetails') {
                                                    const values = delta as unknown as AuditDeviceChangesMap;
                                                    return Object.entries(values).map(([dev, deviceChange]) => {
                                                        const newValue = deviceChange.new;
                                                        const isObject = newValue && typeof newValue === 'object';
                                                        const installationDate = isObject ? newValue.installationDate : undefined;
                                                        ...
                                                        return (
                                                            <div key={dev} className="flex items-center gap-2 text-[10px]">
                                                                ...
                                                                <span className="text-slate-600">{installationDate ? `Instalado ${installationDate}` : 'Retirado'}</span>
                                                            </div>
                                                        );
                                                    });
                                                }
