...
export const useCudyrLogic = (readOnly: boolean) => {
    ...
    const handleScoreChange = useCallback((bedId: string, field: keyof CudyrScore, value: number) => {
        updateCudyr(bedId, field, value);
    }, [updateCudyr]);

    const handleCribScoreChange = useCallback((bedId: string, field: keyof CudyrScore, value: number) => {
