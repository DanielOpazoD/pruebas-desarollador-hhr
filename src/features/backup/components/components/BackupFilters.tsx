...
export const BackupFilters: React.FC<BackupFiltersProps> = ({ filters, onFiltersChange }) => {
    ...
    <div className="bg-white border border-slate-200 rounded-lg p-4 mb-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {Object.keys(BACKUP_TYPE_CONFIG).map(key => {
            const config = getBackupTypePresentation(key as BackupFileType);
            return (
        ...
        >
          ...
          {Object.keys(SHIFT_TYPE_CONFIG).map(key => {
            const config = getBackupShiftPresentation(key as BackupShiftType);
            return (
              <option key={key} value={key}>
                {config.label}
              </option>
