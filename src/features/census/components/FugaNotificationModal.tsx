...
export const FugaNotificationModal: React.FC<FugaNotificationModalProps> = ({
  ...
  }, [isPsychiatry, manualRecipients]);

  const hasInvalidEmails = useMemo(
    () => !isPsychiatry && resolvedRecipients.some(email => !EMAIL_REGEX.test(email)),
    [isPsychiatry, resolvedRecipients]
  );

  const handleSend = async () => {
    setError(null);

    if (!isPsychiatry && resolvedRecipients.length === 0) {
    ...
    try {
      await sendFugaNotification({
        ...
        recipients: isPsychiatry ? undefined : resolvedRecipients,
      });
    ...
    >
      <div className="space-y-3 text-sm text-slate-700">
        <p className="text-xs text-slate-600">
          Se enviará un correo automático con el reporte de fuga. Puedes agregar una nota opcional
          para complementar antecedentes.
        </p>
