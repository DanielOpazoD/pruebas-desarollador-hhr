...
export const hospitalSalvadorConfig: HospitalConfig = {
  ...
  emails: {
    to: ['ugchds@hsalvador.cl', 'traslados@hsalvador.cl'],
    cc: [
...
const normalizeHospitalKey = (value: string): string =>
  value
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
export const getHospitalConfigByDestinationName = (
  destinationHospital: string
): HospitalConfig | undefined => {
  const normalizedDestination = normalizeHospitalKey(destinationHospital);

  return getHospitalConfigs().find(config => {
    const normalizedName = normalizeHospitalKey(config.name);
    const normalizedCode = normalizeHospitalKey(config.code);
    return (
      ...
      normalizedDestination === normalizedCode
    );
  });
};

export const hasTransferDocumentConfig = (destinationHospital: string): boolean =>
  Boolean(getHospitalConfigByDestinationName(destinationHospital));
