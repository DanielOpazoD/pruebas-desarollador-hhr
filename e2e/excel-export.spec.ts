...
test.describe('Export actions in Census', () => {
  ...
  test('shows local Excel export option in Save dropdown', async ({ page }) => {
    ...
    await expect(page.getByText('Descargar Local', { exact: false })).toBeVisible({
      timeout: 10000,
    });
    await expect(page.getByText('Exportar planilla Excel', { exact: false })).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test('shows email Excel option in Send dropdown', async ({ page }) => {
    ...
    await expect(page.getByText('Enviar Archivo Excel', { exact: false })).toBeVisible({
      timeout: 10000,
    });
  });
