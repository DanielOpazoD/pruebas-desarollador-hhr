...
Mario.MapState = function () {
  ...
  // Check saved world progress
  var savedProg = localStorage.getItem('hhr_mario_progress');
  if (savedProg) {
    var p = JSON.parse(savedProg);
    if (p.worldNumber !== undefined) {
      this.WorldNumber = p.worldNumber - 1; // It increments immediately in NextWorld
    }
  }
  this.NextWorld();
};
...
Mario.MapState.prototype.LevelWon = function () {
  ...
  // Guardar avance en Local Storage y notificar a React App superior
  var progressData = {
  ...
  localStorage.setItem('hhr_mario_progress', JSON.stringify(progressData));

  // Si esta inyectado en iframe, pasarlo a la shell.
  if (window.parent !== window) {
    window.parent.postMessage({ type: 'MARIO_LEVEL_WON', data: progressData }, '*');
  }
};
