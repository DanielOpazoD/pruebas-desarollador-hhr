...
const getMissingFields = (config: ReturnType<typeof buildConfig>): string[] =>
  ['apiKey', 'projectId', 'appId'].filter(
...
export const getFirebaseServer = () => {
  if (getApps().length === 0) {
    ...
    if (missingFields.length > 0) {
      throw new Error(
        `Missing Firebase configuration fields: ${missingFields.join(', ')}. Ensure Netlify environment variables are set.`
      );
    }
